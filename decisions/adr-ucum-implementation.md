...
NMDC records measurements in two places: the **submission-schema**, where values appear as a single `{value} {unit}` string, and the **nmdc-schema, for data storage**, where a `QuantityValue` class stores the numeric part in the `has_value` slot and the unit in the `has_unit` slot. Although many slots document a `preferred_unit` (originating from MIxS), most currently accept any unit (e.g., `cm`, `meter`, or even `banana`) in `has_unit`, which breaks data interoperability.
...
- Create various subclasses of `QuantityValue` for slots we want constrained, for example `QuantityValueTemp` where `has_unit` is required and constrained to 'C'. Downside to this approach was that a separate `QuantityValue` subclass had to be made for each `has_unit` value we wanted to require.
- Create subclass `QuantityValuewithUnit` where `has_unit` is a required slot and constrained to enum `UnitEnum`. Downside to this approach was that any of the `UnitEnum` values could be used for any slot (i.e., 'm' could be used for `temp` slot).
