...
- **LinkML validator plugin** – A custom LinkML validator plugin (`NmdcSchemaValidationPlugin`) has been written that validates instance data against the `storage_units` constraints, ensuring that submitted data only uses allowed UCUM symbols for each slot. 
