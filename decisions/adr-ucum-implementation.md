...
- **Schema annotation tests** - We wrote automated tests (`tests/test_units_alignment.py`) that check each `QuantityValue` slot has a `storage_units` annotation. A small number of slots currently retain temporary `units_alignment_excuse` annotations (e.g., `api`, `efficiency_percent`, `inside_lux`) for cases requiring resolution of MIxS specification inconsistencies or analysis of complex/non-UCUM units. The goal is to eliminate all excuse annotations by providing proper UCUM `storage_units` for every slot or converting non-complinat units to non-QuantifyValue range so all QuantityValue ranges are UCUM compliant.
