...
- The `storage_units` annotation is **mandatory** for every `QuantityValue` slot. `units_alignment_excuse` was allowed during development and should not be added to any subsequential `QuantityValue` slots.
...
1. **Runtime validation** – The `units‑ucum‑validate` test (`tests/test_units_alignment.py`) iterates over all slots, verifies that each `QuantityValue` slot either has a non‑empty `storage_units` or a valid `units_alignment_excuse`.
2. **CI integration** – The test suite runs on every PR; failures block merging until the annotation is added.
